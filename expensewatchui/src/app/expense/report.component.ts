...
export class ReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.forEach((params: Params) => {
      ...
      this.qsort = params['sortby'] || 'expensedate';
  ...
  fetchReport(userid, formval) {
    ...
    .subscribe(data => {
      if (data.success === false) {
        ...
        this.expenses = JSON.parse(data.data);
        this.totalrows = +data.totalrows;
        this.exptotal = +data.exptotal;
  ...
  confirmDel(idx: number, expid: number, expamt: number) {
    if (confirm('Do you really want to delete this record?')) {
      ...
      .subscribe(data => {
        if (data.success === false) {
          ...
          this.exptotal = this.exptotal - expamt;
